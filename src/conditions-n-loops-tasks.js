...
function canQueenCaptureKing(queen, king) {
  ...
  if (Math.abs(queen.y - king.y) === Math.abs(queen.x - king.x)) return true;
  return false;
}
...
function convertToRomanNumerals(num) {
  ...
  while (numTest > 0) {
    while (numTest >= Roman[i].value) {
      res += Roman[i].symbol;
      numTest -= Roman[i].value;
    }
    i += 1;
  }
...
function rotateMatrix(matrix) {
  ...
  const size = rotMatrix.length;
  const n = size - 1;

  for (let i = 0; i < size / 2; i += 1) {
    for (let j = i; j < n - i; j += 1) {
      const temp = rotMatrix[i][j];
      rotMatrix[i][j] = rotMatrix[n - j][i];
      rotMatrix[n - j][i] = rotMatrix[n - i][n - j];
      rotMatrix[n - i][n - j] = rotMatrix[j][n - i];
      rotMatrix[j][n - i] = temp;
    }
  }
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < n; i += 1) seen[i] = false;
  ...
  for (let i = 0; i < n; i += 1) {
    if (!seen[i]) {
      ...
      while (!seen[curr]) {
        ...
        cycleLen += 1;
        seen[curr] = true;
        curr = curr % 2 === 0 ? curr / 2 : half + Math.floor(curr / 2);
      }

      for (let j = 0; j < cycleLen; j += 1) {
        const pos = cycle[j];
        const newPos = cycle[(j + iterations) % cycleLen];
        res[newPos] = str[pos];
      }
    }
  }
...
function getNearestBigger(number) {
  ...
  while (pivotIdx >= 0 && digits[pivotIdx] >= digits[pivotIdx + 1]) {
    pivotIdx -= 1;
  }
  if (pivotIdx < 0) return number;

  let swapIdx = digits.length - 1;
  while (digits[swapIdx] <= digits[pivotIdx]) {
    swapIdx -= 1;
  }

  [digits[pivotIdx], digits[swapIdx]] = [digits[swapIdx], digits[pivotIdx]];

  const right = digits.splice(pivotIdx + 1).sort((a, b) => a - b);
  digits.push(...right);
